...
def main():
    ...
    for seq in sequences:
        ...
        path = os.path.join(args.dataset_path, seq, "frame{0:06d}.png")
        ...
        camera_matrix = np.loadtxt(os.path.join(args.dataset_path, seq, 'cam.txt')).astype(np.float32)
        # camera_matrix[0, :] /= 620
        # camera_matrix[1, :] /= 288
        # print(camera_matrix)
        rvec = np.array([0., 0., 0])
        ...
        for i in tqdm(range(len(pose))):
            ...
            save_path = '{}'.format(seq) + '.frame{0:06d}.png'.format(i)
            print(save_path, sum_euler, category)
